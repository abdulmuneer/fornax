...
- Local Fornax evidence on 2026-07-01 showed that a source-built MAX checkout
  (`external/modular`, `MAX 26.5.0.dev2026063006`) can run short
  `DeepSeek-V2-Lite-Chat` generation on an M3 Max after Apple MLA prefill,
  Apple MLA decode fallback, Apple MoE indices fallback, Metal dispatch scalar,
  and rank-2 gather patches. Treat this as local build evidence, not packaged
  MAX support or serving-grade validation.
