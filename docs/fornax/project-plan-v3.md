...
2026-07-01 update: a rank-1 local probe in the vendored `external/modular` MAX
checkout showed that source-built `MAX 26.5.0.dev2026063006` can run short
`DeepSeek-V2-Lite-Chat` generation on an M3 Max after local Apple MLA prefill,
Apple MLA decode fallback, Apple MoE index fallback, Metal dispatch-scalar, and
rank-2 gather patches. This strengthens the Apple/MAX substrate evidence but
does **not** close G1: the result is not packaged MAX support, not distributed
Fornax serving, not numerical parity, and not the target expert-MLP probe that
assigns Apple's v0 role. See
