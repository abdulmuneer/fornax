...
Derived from plan **v4** §4–§8. Each epic rolls up to a
workstream ([01](01-stakeholders-and-raci.md)) and a gate ([04](04-stage-gates.md)).
Implementation boundaries follow the v4 ownership boundary plus the preserved
module map from plan v3 §5.3
...
- A5 **`v0-target-contract.md`** (closes B1/B2; seeded by §3.2) — accepted at G1
- A6 **Phase-0 preflight workflow** (§3.4): inventory → fabric probe → target
  validate → plan → simulate → benchmark → doctor (owner DIST+SRE) — G1 baseline
- A7 Fix expert-host capacity/resource accounting and add the reproduced
  false-feasibility regression — **complete at T0**.
...
- D4 Reversal-trigger evaluation feeding physical Apple role assignment — §5.5
...
- E5 Stage ABI v1 binary framing, credit, ordering, acknowledgement, and
  malformed-frame conformance — **complete at T0/T1**.
...
Engine v0 has exited, but DEC-007's freeze on new Phase 3-5 proxy expansion
remains until a reviewed v4/G2 reprioritization releases it. Existing fixtures
remain regression inputs; they do not outrank J3/J5 evidence.
