...
| S3-7 | Partial: state-ownership simulation exists; localhost mTLS/HTTPS lifecycle cleanup, local target-fixture parity, and a 5/5 H100 local serving-runtime bundle with integrated target-fixture execution evidence exist; live distributed lifecycle proof remains open. |
...
- `python3 -m fornax program local-http-serving-smoke --backend-mode target-fixture --enable-mtls --out /tmp/fornax_local_http_serving_mtls_target_fixture_smoke_20260622.json`
...
- Security, topology, replication, lifecycle, local mTLS/HTTPS target-fixture parity, measured local H100 fixture execution, and the integrated local serving-runtime target-fixture bundle are
