...
### Apple Silicon MAX/Qwen MoE serving smoke milestone

- Created and switched to branch `fornax_apple_silicon`.
- Used the `last30days` skill contract as requested. The skill is installed, but
  its engine reported first-run setup is incomplete. Per the skill's consent
  rules, browser-cookie backed X/Twitter coverage cannot be enabled silently.
  Native web search was used for the official/current MAX Apple Silicon check
  while waiting for user consent on last30days setup.
- Official MAX source check:
  - MAX model support in the installed nightly lists `Qwen3MoeForCausalLM`,
    `DeepseekV3ForCausalLM`, `KimiK25ForConditionalGeneration`, and
    `GlmMoeDsaForCausalLM` families.
  - The 26.4 public release notes expanded Apple Silicon support directionally,
    while current quickstart/docs still make datacenter GPUs the safer default
    and local probes the source of truth.
  - Local source precedence still stands: this Mac's measured/probed behavior
    decides Apple role assignment, not blog wording.
- Local Mac facts discovered:
  - Hardware: MacBook Pro `Mac15,8`, Apple M3 Max, 16 CPU cores, 128 GB unified
    memory, Darwin 25.5.0.
  - The pre-existing `modular` CLI is `0.9.3` and Mojo is `24.3.0`; `max` was
    not on `PATH`.
  - `modular install max --no-checks` failed with an expired manifest and noted
    that the legacy `modular` CLI is deprecated in favor of `magic`.
  - A throwaway `pixi` environment under `/tmp/fornax-max-smoke.1CqC8m` installed
    current Modular/MAX from the nightly channel: `MAX
    26.5.0.dev2026062906` and `Mojo 1.0.0b3.dev2026062906`.
- Model target discovery:
  - `Qwen/Qwen3-30B-A3B-Instruct` appears in the MAX model-list example but did
    not resolve on Hugging Face from this machine.
  - `Qwen/Qwen3-30B-A3B` and `Qwen/Qwen3-Coder-30B-A3B-Instruct` both resolve and
    have `architectures=["Qwen3MoeForCausalLM"]`, `model_type="qwen3_moe"`,
    `num_experts=128`, and `num_experts_per_tok=8`.
  - A local MTPLX-optimized Qwen3.6-27B artifact exists, but it is dense
    (`Qwen3_5ForConditionalGeneration`) and therefore does not satisfy the real
    frontier MoE test.
- Implementation:
  - Added `fornax.apple_silicon_moe_serving_smoke` and CLI command
    `fornax program apple-silicon-moe-serving-smoke`.
  - The command runs a real Qwen/DeepSeek/Kimi/GLM MoE through `max generate`,
    targets Apple GPU via `--devices gpu`, reads the cached Hugging Face
    `config.json`, records MAX/Mojo versions, records sanitized Apple hardware
    metadata, wraps the string output in an OpenAI-compatible response-shaped
    artifact, and keeps all non-closure claims explicit.
  - Added `--max-cwd` after discovering that Pixi-installed MAX must be invoked
    as `pixi run max` from the Pixi project directory. Calling the raw environment
    binary directly can fail earlier while resolving built-in MAX kernel packages.
  - Added repeatable `--max-extra-arg` so MAX bring-up flags can be captured in
    structured Fornax artifacts instead of one-off shell commands.
  - Added `fornax test apple-silicon-moe-serving-smoke` to validate saved
...
- Review-lens pass against `../research/combined_skill_groups.md`:
  - Hardware/Apple: fixed the important issue that a dense local Qwen artifact
    ...
    validator.
  - Low-level/MAX: fixed source-of-truth drift by recording the exact MAX/Mojo
    runtime and rejecting artifacts without a `MAX ...` version.
  - Build/toolchain: fixed the Pixi invocation footgun with `--max-cwd` so the
    wrapper exercises MAX's intended runtime environment before judging Apple
    graph compilation behavior.
  - Runtime bring-up: added a raw MAX flag escape hatch after MAX suggested
    `--no-enable-overlap-scheduler --force` as a possible override path.
  - LLM correctness: kept the output at string-in/string-out serving-smoke scope;
    the artifact does not claim logit/layer parity.
  - Software engineering/testing: added focused validator tests for valid Qwen
    MoE metadata, dense-model rejection, and gate-overclaim rejection.
  - Documentation: updated user-facing commands and limitations so users do not
    infer distributed serving or G2/G3 closure from a single-Mac smoke.
- Verification so far:
  - `python3 -m py_compile fornax/apple_silicon_moe_serving_smoke.py
    fornax/cli.py tests/test_fornax_planner.py`
  - Focused unittest selectors for the four new Apple Silicon MoE smoke tests.
  - `python3 -m fornax program apple-silicon-moe-serving-smoke --help`
  - `python3 -m fornax test --help`
  - `python3 -m compileall -q fornax tests`
  - `make test` passes when run outside the socket-restricting sandbox: golden
    and contract checks passed, followed by 261 unittest cases. The sandboxed
    run failed only on existing local HTTP smoke tests that bind a localhost
    socket.
- Live hardware/model smoke:
  - Direct `max generate` against `Qwen/Qwen3-30B-A3B` was launched from the
    temporary MAX environment with `--devices=gpu` and `--max-new-tokens 8`.
    It downloaded all 16 safetensor shards, then failed during Apple Metal graph
    compilation: `Metal Compiler failed to compile metallib` and `graph compiler
    could not elaborate the generated KGEN`.
  - Re-running through the Fornax wrapper with `--local-files-only`,
    `--max-command "pixi run max"`, and `--max-cwd
    /private/tmp/fornax-max-smoke.1CqC8m` produced structured evidence at
    `/tmp/fornax_apple_qwen3_moe_smoke.json`. The artifact correctly records
    `Qwen3MoeForCausalLM`, `num_experts=128`, `num_experts_per_tok=8`, `MAX
    26.5.0.dev2026062906`, and `Apple M3 Max`, but it does not pass because MAX
    returns nonzero before emitting generated text.
  - `fornax test apple-silicon-moe-serving-smoke --fixture
    /tmp/fornax_apple_qwen3_moe_smoke.json` correctly fails the artifact with
    `ok must be true`, `serving.generated_text must be a non-empty string`, and
    `result.returncode must be 0`.
  - Retrying MAX with `--no-enable-overlap-scheduler --force` still failed with
    the same Metal/KGEN graph compilation signature, so overlap scheduling is
    not the only blocker on this Apple GPU path.
  - Conclusion: on this M3 Max, Fornax can identify, launch, and validate the
    real frontier MoE smoke path, but current nightly MAX cannot yet serve
    `Qwen/Qwen3-30B-A3B` on Apple GPU to generated text. This remains bring-up
    evidence and does not close Apple real-MoE serving.
...
### Phase 3 two-H100 proxy gate pass

- Added `fornax.phase3_proxy_gate`, a machine-checkable Phase 3/G3 proxy packet builder and validator. It consumes the passing local HTTP endpoint/runtime/topology-route artifact, verifies the endpoint artifact, two-H100 logical-host probes, local mTLS/auth/plan checks, local failure semantics, lifecycle cleanup, runtime probes, topology route, and explicit no-formal-G3-overclaim boundary.
- Added `python3 -m fornax program phase3-proxy-gate` to write the proxy packet. The command records `phase3_proxy_passed=true` only when the operator-selected outcome is `PROCEED` and all proxy checks pass; it always keeps `formal_g3_passed=false` and records formal G3 validation as deferred.
- Generated `/tmp/fornax_phase3_g3_two_h100_proxy_gate_20260623.json` from `/tmp/fornax_local_http_serving_transport_runtime_bundle_mtls_target_fixture_topology_route_smoke_20260623.json`. The proxy packet passed 8/8 checks, references the 23/23 endpoint artifact, uses `localhost`, `logical-host-0`, and `logical-host-1` backed by two H100 GPUs, and records deferred requirements for real frontier target-model loading/parity, AMD GPU node, Apple Silicon Mac, product auth/mTLS keying, and distributed partition proof.
- Updated the tracked sprint/status/index docs to mark Phase 3 complete at the accepted two-H100 proxy scope. This follows the operator decision that actual NVIDIA/AMD/Mac validation can wait and should not block the current gate pass.
- Review-lens pass:
  - System Engineering: approve with comments. The proxy gate is explicit about scope and evidence boundaries instead of silently redefining the formal heterogeneous lab requirement.
  - Hardware Acceleration: approve with comments. The gate uses measured two-H100 CUDA evidence for the current pass and tracks AMD/Apple as deferred validation.
  - Networking/Security: approve with comments. Local mTLS/auth/plan integrity and failure semantics are required proxy checks; product keying remains deferred.
  - Low-level Software: approve. The validator keeps endpoint validation, proxy evidence checks, and formal-G3 deferrals mechanically separate.
  - Software Engineering: approve. The CLI command and unit tests make the gate packet reproducible and prevent formal-G3 overclaiming.
  - Testing/Quality: approve. Focused proxy tests, compile, CLI packet generation, and diff hygiene passed before commit; broader Fornax validation remains the final pre-commit step.
  - Program Management: approve. Phase 3 now has an auditable PROCEED record at two-H100 proxy scope with deferred formal lab items named.
- Verification: `python3 -m py_compile fornax/phase3_proxy_gate.py fornax/cli.py tests/test_fornax_planner.py`, focused proxy gate tests showing 2/2 passed, `python3 -m fornax program phase3-proxy-gate --endpoint-artifact /tmp/fornax_local_http_serving_transport_runtime_bundle_mtls_target_fixture_topology_route_smoke_20260623.json --out /tmp/fornax_phase3_g3_two_h100_proxy_gate_20260623.json --date 2026-06-23 --outcome PROCEED --accepted-by operator` showing 8/8 proxy checks passed, `python3 -m unittest tests.test_fornax_planner` showing 226 tests passed, `python3 -m compileall -q fornax tests`, `make fornax-golden`, `make fornax-test`, and `git diff --check` all passed.

### Phase 4 two-H100 resilience proxy gate pass

- Added `fornax.phase4_resilience_gate`, a machine-checkable Phase 4/G4 proxy packet builder and validator. It composes the existing resilience replay, stage-replication, and ops-lifecycle artifacts with a T4 runbook, then records `phase4_proxy_passed=true` only when all proxy checks pass and the operator outcome is `PROCEED`.
- Added `python3 -m fornax program phase4-resilience-gate` with optional `--runbook-out`, plus `python3 -m fornax test phase4-resilience-gate` and a golden fixture at `fornax/golden_vectors/phase4_resilience_gate/fixture.json`.
- Added the tracked T4 runbook `docs/fornax/program_management/t4-resilience-runbook.md`. It defines evidence collection for single-node-loss zero-drop replay, added-node scaling, drain/restart/rollback hooks, and later heterogeneous lab follow-up.
- Generated `/tmp/fornax_phase4_g4_two_h100_proxy_gate_20260623.json` and `/tmp/fornax_phase4_t4_resilience_runbook_20260623.md`. The proxy packet passed 7/7 checks with `phase4_proxy_passed=true`, `formal_g4_passed=false`, four runbook scenarios, zero dropped in-flight requests, replication speedup/parity, and lifecycle rollback/node replacement verified.
- Boundary remains explicit: this is the accepted two-H100 local proxy gate for current development. Formal T4/G4 remains deferred for real single-node-loss fault injection, real added-node scaling, AMD GPU, Apple Silicon Mac, and production drain/restart hooks.
- Review-lens pass:
  - System Engineering: approve with comments. The packet composes replay, added capacity, lifecycle hooks, and runbook evidence into one auditable gate artifact while preserving formal T4 deferrals.
  - Hardware Acceleration: approve with comments. Two local H100 GPUs are used as logical-host stand-ins per operator instruction; AMD/Mac evidence remains explicitly deferred.
  - Networking/Security: approve with comments. Timeout/retry/cancel/partition semantics are covered through existing local endpoint and T1 artifacts, but product transport/auth fault injection remains future work.
  - Low-level Software: approve. The validator keeps artifact validation, proxy evidence checks, runbook requirements, and formal-G4 boundary checks mechanically separate.
  - Software Engineering: approve. CLI, golden fixture, and focused tests make the proxy packet reproducible and reject formal-G4 overclaims.
  - Testing/Quality: approve with comments. Focused Phase 4 tests, CLI validation, and compile checks passed before broader validation; full suite and golden validation remain the final pre-commit checks.
  - Program Management: approve. Phase 4 now has an auditable PROCEED record at two-H100 proxy scope and a tracked T4 runbook, with formal lab closure items named.
- Verification: `python3 -m py_compile fornax/phase4_resilience_gate.py fornax/cli.py tests/test_fornax_planner.py` passed; focused Phase 4 unit tests passed 4/4; `python3 -m fornax test phase4-resilience-gate` passed; `python3 -m fornax program phase4-resilience-gate --resilience-artifact fornax/golden_vectors/resilience_replay --replication-artifact fornax/golden_vectors/stage_replication --ops-artifact fornax/golden_vectors/ops_lifecycle --out /tmp/fornax_phase4_g4_two_h100_proxy_gate_20260623.json --runbook-out /tmp/fornax_phase4_t4_resilience_runbook_20260623.md --date 2026-06-23 --outcome PROCEED --accepted-by operator` passed 7/7 proxy checks; `python3 -m unittest tests.test_fornax_planner` passed 230 tests; `python3 -m compileall -q fornax tests` passed; `make fornax-golden` passed including `phase4-resilience-gate`; `make fornax-test` passed 230 tests; `git diff --check` passed.

### Phase 4 review-lens fix and Phase 5 two-H100 productization proxy gate

- Review-lens issue fixed: the Phase 4/G4 packet previously implied two-H100 proxy scope through artifact provenance but did not explicitly record selected local H100 devices or logical hosts. Added `proxy_hardware` metadata, CLI flags, fixture coverage, and validator checks so Phase 4 now rejects missing or single-device proxy evidence while keeping `formal_g4_passed=false`.
- Added `fornax.phase5_ga_gate`, a machine-checkable Phase 5/G5 productization proxy packet builder and validator. It composes ops lifecycle, onboarding methodology, benchmark ledger, prior G4 proxy evidence, G5 runbook scenarios, and explicit H100 proxy metadata into an 11/11 proxy packet.
- Added `python3 -m fornax program phase5-ga-gate` with `--runbook-out`, `python3 -m fornax test phase5-ga-gate`, and the golden fixture `fornax/golden_vectors/phase5_ga_gate/fixture.json`.
- Added the tracked G5 runbook `docs/fornax/program_management/g5-productization-runbook.md` and updated sprint/index/status docs. The Phase 5 sprint is passed at accepted two-H100 proxy scope, with installable release, lab-reference benchmark-of-record, fresh operator onboarding, production lifecycle hooks, and Sponsor GA acceptance deferred.
- Generated `/tmp/fornax_phase5_g5_two_h100_proxy_gate_20260623.json` and `/tmp/fornax_phase5_g5_productization_runbook_20260623.md`. The proxy packet passed 11/11 checks with `phase5_proxy_passed=true`, `formal_g5_passed=false`, six runbook scenarios, lifecycle zero dropped in-flight requests, onboarding docs/glossary/methodology present, a measured benchmark-ledger record, prior G4 proxy attached, and `cuda:0`/`cuda:1` recorded as local H100 stand-ins.
- Review-lens pass:
  - System Engineering: approve with comments. Phase 5 now packages operator UX, lifecycle, node replacement, onboarding, benchmark, and prior resilience evidence into one auditable proxy gate while preserving formal Product GA deferrals.
  - Software Engineering: approve. Builder, validator, CLI, golden fixture, and focused tests make the gate reproducible and reject formal-G5 overclaims.
  - Hardware Acceleration: approve with comments. Proxy scope records selected local H100 devices and logical hosts; AMD/Mac and lab-reference hardware remain deferred instead of blocking current development.
  - Networking/Security: approve with comments. Operator configs require auth, plan integrity, and serving endpoint coverage, but production keying and distributed evidence remain formal follow-up.
  - Testing/Quality: approve. Focused Phase 4/5 tests, CLI gate generation, full unit suite, compileall, Makefile golden/test, and diff hygiene passed.
  - Program Management: approve. All planned sprint phases now have machine-checkable proxy evidence at the accepted local scope; formal gate closure lanes remain named and unblocked for later validation.
- Verification: `python3 -m py_compile fornax/phase5_ga_gate.py fornax/phase4_resilience_gate.py fornax/cli.py tests/test_fornax_planner.py` passed; focused Phase 5 unit tests passed 5/5; `python3 -m fornax test phase5-ga-gate` passed; `python3 -m fornax test phase4-resilience-gate` passed; `python3 -m fornax program phase5-ga-gate --ops-artifact fornax/golden_vectors/ops_lifecycle --onboarding-artifact fornax/golden_vectors/onboarding_methodology --benchmark-ledger fornax/golden_vectors/benchmark_ledger --phase4-artifact fornax/golden_vectors/phase4_resilience_gate --out /tmp/fornax_phase5_g5_two_h100_proxy_gate_20260623.json --runbook-out /tmp/fornax_phase5_g5_productization_runbook_20260623.md --date 2026-06-23 --outcome PROCEED --accepted-by operator --proxy-hardware-name 'NVIDIA H100 80GB HBM3' --proxy-devices cuda:0,cuda:1 --proxy-logical-hosts logical-host-0,logical-host-1` passed 11/11 proxy checks; `python3 -m unittest tests.test_fornax_planner` passed 236 tests; `python3 -m compileall -q fornax tests` passed; `make fornax-golden` passed including `phase5-ga-gate`; `make fornax-test` passed 236 tests; `git diff --check` passed.

### Phase 4/5 gate validator review-lens hardening

- Reviewed the Phase 4 and Phase 5 proxy gate packets through the Software Engineering, Low-level Software, System Engineering, Hardware, and Networking review lenses.
- Top issue fixed: malformed packet content could crash validators instead of returning structured errors. Replaced unsafe `set(...)` and non-object `.get(...)` assumptions with total validation for proxy device/host lists and evidence-check entries.
- Top issue fixed: artifact chain-of-custody was too loose inside gate packets. `artifact_validation` entries now carry explicit artifact paths, and validators require them to match the top-level `artifacts` map for Phase 4 and Phase 5 packets.
- Regenerated the Phase 4 and Phase 5 golden gate fixtures so the path ledger is present in both proxy packets.
- Added regression tests for malformed evidence checks, malformed proxy hardware, and artifact-validation mismatches.
- Review-lens pass:
  - Software Engineering: approve. Validators now fail closed with actionable errors instead of exceptions, and test coverage pins the behavior.
  - Low-level Software: approve. Packet parsing is robust against malformed JSON shapes and unhashable values.
...
- Top review findings implemented:
  - Phase 3 proxy-gate validation is now aligned with the stricter Phase 4/5
    gate validators: it checks gate date/signature metadata, endpoint validation
    shape, formal deferred-requirement statuses/reasons, and only allows
    `phase3_proxy_passed=true` for a fully valid proxy packet.
  - Added packaged Phase 3 proxy-gate golden evidence under
    `fornax/golden_vectors/phase3_proxy_gate/`, plus `fornax test
    phase3-proxy-gate`, so G3 proxy validation no longer depends on prior
    `/tmp` artifacts.
  - Expanded `make golden` to run the deterministic no-hardware CLI
    contract/golden suites, including G3/G4/G5 proxy-gate fixtures and the
    current scheduler, transport, MoE, observability, ops, onboarding, backend
    coverage, benchmark, pipeline, and throughput fixtures.
  - Updated README and CLI reference status text to reflect Phase 3-5 proxy
    evidence while preserving formal G1-G5 deferrals.
- Review artifact: `docs/fornax/review_lens_pass_2026_06_29.md`.
- Verification: `python3 -m py_compile fornax/phase3_proxy_gate.py fornax/cli.py
  tests/test_fornax_planner.py`; focused Phase 3 unittest selection (6 tests);
  `python3 -m fornax test phase3-proxy-gate`; `make golden`; `python3 -m
  unittest discover -s tests -p 'test_fornax*.py'` (250 tests); and `make
  test` all passed.
- Remaining formal gaps are unchanged: real frontier target-model parity,
  production auth/mTLS keying, real distributed partition proof, real
  node-loss/add-node lab evidence, installable release packaging,
  lab-reference benchmark-of-record, fresh operator acceptance, and Sponsor
  decisions remain deferred.

#### Learnings from this pass

- Review lenses are most useful when they compare acceptance bars across adjacent
  gates, not only inside one module. The Phase 3 proxy gate looked honest in
  isolation, but the Phase 4/5 validators had already raised the standard for
  decision metadata, deferred-requirement invariants, and fail-closed packet
  consistency. The fix was to make the acceptance bar uniform.
- Packaged golden fixtures are stronger evidence than references to prior `/tmp`
  runs. `/tmp` artifacts remain useful provenance for hardware/proxy runs, but a
  CI-friendly fixture is what keeps a gate validator reproducible after the
  original workstation context disappears.
- The default validation command is part of the governance surface. Once most
  deterministic contract suites lived behind `fornax test <name>`, keeping
  `make golden` on the old small subset created status drift: developers could
  run the advertised command and miss failures in newer gate packets.
- Proxy-vs-formal warnings should be treated as product behavior, not noise.
  The warnings in the expanded golden run are doing useful work: they preserve
  local two-H100 progress while stopping readers from converting proxy evidence
  into NVIDIA/AMD/Mac, production security, or Sponsor-GA closure.
- Documentation drift is easiest to catch from the user-entrypoint lens. The
  README lagged the program ledger even though the code and status file were
  precise; public status text needs the same proxy/formal distinction as the
  gate packets.
- The remaining maintainability risks are now clearer: `cli.py`,
  `local_http_serving_smoke.py`, and the monolithic test module still work, but
  the next scale-up should split command surfaces and per-module oracle tests
  before adding much more gate logic.

### 4-GPU MoE serving investigation learnings

- The local machine can expose four H100 GPUs to Fornax, but the existing
  same-host serving evidence does not yet prove a four-GPU MoE serving path.
  `nvidia-smi` showed GPUs 0-3 present, with GPU 0 already carrying a large
  resident allocation and GPUs 1-3 effectively free; a useful smoke test should
  therefore report exact device placement, memory state, and which devices
  actually executed expert work instead of only listing available accelerators.
- The current `local_serving_smoke` bundle is valuable as a composite contract
  check, but it is not a live multi-GPU MoE server proof. It combines the
  serving adapter fixture, pipeline checks, MoE parity, and target fixture
  execution while explicitly marking `live_http_endpoint=false`,
  `target_model_parity=false`, and `g2_g3_gate_evidence=false`. A 4-GPU MoE
  smoke should preserve that honesty pattern unless it really starts a live
  endpoint and exercises a real target model.
- The existing MoE parity probe is a two-device, same-host simulation: one
  source CUDA device plus one expert CUDA device. It is the right correctness
  pattern to reuse because it compares the split execution path against a CPU or
  single-device reference and records route traces, but it must be generalized
  before it can answer the 4-GPU serving question.
- The correct next proof shape is a deterministic tiny MoE serving fixture with
  one gateway/source GPU and three expert GPUs, route coverage proving every
  expert GPU received work, split-vs-reference logit/token parity, throughput
  timing, and explicit non-claims for formal G2/G3 closure, frontier-model
  parity, and production distributed serving.
- The user guide currently emphasizes the no-GPU developer path. That remains
  the right default, but the guide needs an optional hardware section once the
  4-GPU smoke exists so operators can run the exact command and understand that
  the result is same-host H100 proxy evidence, not formal heterogeneous cluster
  validation.

### 4-GPU MoE serving smoke implementation and review (2026-06-29)

- Implemented `fornax program local-4gpu-moe-serving-smoke`, backed by
  `fornax/local_4gpu_moe_serving_smoke.py`, to exercise a deterministic tiny MoE
  serving fixture across four CUDA devices: one gateway/router/gather GPU and
  three expert GPUs. The probe compares split GPU execution against a reference
  path, emits an OpenAI-shaped fixture response, records route traces, expert
  placement, per-expert-GPU work counts, peer-access metadata, memory snapshots,
  throughput, and explicit non-claims.
- Added `fornax test local-4gpu-moe-serving-smoke --fixture ...` so saved
  hardware artifacts can be validated without rerunning CUDA. Added focused
  unit coverage for the artifact contract: valid four-GPU coverage, rejection
  when an expert GPU receives no work, and rejection of formal G3 overclaims.
- Live hardware evidence: `/mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python`
  reports PyTorch `2.12.0+cu130`, CUDA available, and four `NVIDIA H100 80GB
  HBM3` devices. `nvidia-smi` before the run showed `cuda:0` with about 46 GiB
  resident and `cuda:1-3` effectively free, so the smoke used `cuda:0` as the
  gateway and `cuda:1,cuda:2,cuda:3` as expert GPUs.
- Live validation command:
  `python3 -m fornax program local-4gpu-moe-serving-smoke --out-dir
  /tmp/fornax_local_4gpu_moe_serving_smoke_20260629 --torch-python
  /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --devices
  cuda:0,cuda:1,cuda:2,cuda:3 --iterations 5 --warmup 1 --token-count 6
  --hidden-dim 16 --intermediate-dim 32 --vocab-size 17 --expert-count 6
  --top-k 2 --dtype float32 --tolerance 0.0001 --timeout-s 180` passed `3/3`.
  The saved artifact reports `gpu_count=4`, gateway `cuda:0`, expert devices
  `cuda:1,cuda:2,cuda:3`, per-iteration expert-device calls `4/4/4`, 60 total
  expert calls, `tokens_s=1804.82`, `expert_calls_s=3609.64`, generated fixture
  text `tok2 tok11 tok10 tok1 tok5 tok5`, and max layer/logit abs error `0.0`.
- Saved bundle validation command:
  `python3 -m fornax test local-4gpu-moe-serving-smoke --fixture
  /tmp/fornax_local_4gpu_moe_serving_smoke_20260629` passed and preserved the
  warning that this is same-host proxy evidence only.
- Combined skill-groups review findings applied:
  - Hardware / acceleration: the artifact now names the exact devices, roles,
    memory snapshots, peer-access matrix, route placement, and per-expert-GPU
    call counts so "uses 4 GPUs" is machine-checkable.
  - Low-level / LLM correctness: the probe requires split-vs-reference layer and
    logit parity, next-token equality, positive timing, and all expert calls on
    expert GPUs; validators fail closed on missing expert-device coverage.
  - High-level / documentation: the committed getting-started guide, CLI
    reference, and README now show the exact optional hardware command and state
    that it is not the no-GPU default workflow.
  - Networking / system / governance: the bundle and probe enforce
    `live_http_endpoint=false`, `real_frontier_model=false`,
    `target_model_parity=false`, `formal_g2_passed=false`,
    `formal_g3_passed=false`, `g2_g3_gate_evidence=false`, and
    `production_distributed_serving=false`, preventing the local same-host smoke
    from being misread as formal distributed or frontier-model evidence.
- Remaining formal gaps are unchanged: this is a same-host tiny-fixture MoE
  serving proof, not live HTTP endpoint serving, real frontier target-model
  parity, production distributed transport, or formal G2/G3 closure.

### Real Qwen MoE correction after fixture-only finding (2026-06-29)

- The user correctly challenged the prior 4-GPU result: it served
  `fornax-tiny-4gpu-moe-fixture`, not a real frontier MoE checkpoint. That means
  the previous goal was not complete despite the synthetic four-GPU path being
  useful engineering evidence.
- Found a complete local cached checkpoint for
  `Qwen/Qwen3-Omni-30B-A3B-Instruct` at
  `/mnt/dataprocessing/cache/huggingface/hub/models--Qwen--Qwen3-Omni-30B-A3B-Instruct/snapshots/26291f793822fb6be9555850f06dfe95f2d7e695`.
  The snapshot has 15 safetensor shards plus config/tokenizer/template files.
- Local config facts: architecture `Qwen3OmniMoeForConditionalGeneration`,
  `model_type=qwen3_omni_moe`; thinker text stack has 48 layers, 128 experts,
  8 experts per token, hidden size 2048; talker text stack has 20 layers, 128
  experts, 6 experts per token, hidden size 1024. This satisfies the real Qwen
  MoE requirement much better than the tiny fixture.
- Environment facts: `/mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python`
  has PyTorch `2.12.0+cu130`, Transformers `5.8.0.dev0`, CUDA available, and
  four visible H100 GPUs. It does not have vLLM or `qwen_omni_utils`; the
  Qwen3-Omni processor also needs missing `torchvision`, so the text-only smoke
  uses `AutoTokenizer` plus cached `chat_template.json` and calls
  `Qwen3OmniMoeForConditionalGeneration.generate(return_audio=False)` directly.
- Direct local-files-only proof command loaded the real checkpoint with BF16 and
  `device_map=auto`, capping GPU0 at 24 GiB because another resident process is
  using about 46 GiB there. The model loaded in about 205.7 s and generated 24
  tokens in about 11.46 s. Output text began: `MoE stands for Mixture of Experts,
  a technique in AI inference...`.
- Direct proof artifact: `/tmp/fornax_qwen3_omni_4gpu_direct_smoke_20260629.json`.
  It reports `used_devices=[cuda:0,cuda:1,cuda:2,cuda:3]`, device-map counts
  `{0:15, 1:15, 2:15, 3:11}`, 33 prompt tokens, 24 new tokens, about 2.10
  tokens/s, and all four GPU names as `NVIDIA H100 80GB HBM3`.
...
- Promoted the ad hoc Qwen proof into `fornax program local-real-moe-serving-smoke`
  plus `fornax test local-real-moe-serving-smoke --fixture ...`. The command is
  intentionally offline/local by default and validates only saved artifacts for the
  test subcommand, so no no-hardware golden suite accidentally tries to load a 30B
  checkpoint.
- Served `Qwen/Qwen3-Omni-30B-A3B-Instruct` from the cached snapshot
  `/mnt/dataprocessing/cache/huggingface/hub/models--Qwen--Qwen3-Omni-30B-A3B-Instruct/snapshots/26291f793822fb6be9555850f06dfe95f2d7e695`
  with `/mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python`, BF16,
  `device_map=auto`, `CUDA_VISIBLE_DEVICES=0,1,2,3`, GPU0 capped to 24 GiB, and
  the other three GPUs capped to 70 GiB each.
- Fornax artifact `/tmp/fornax_qwen3_omni_real_moe_smoke_20260629.json` validated
  successfully. It records architecture `Qwen3OmniMoeForConditionalGeneration`,
  `model_type=qwen3_omni_moe`, thinker text MoE 48 layers / 128 experts / 8
  experts per token, talker text MoE 20 layers / 128 experts / 6 experts per
  token, `used_devices=[cuda:0,cuda:1,cuda:2,cuda:3]`, device-map counts
  `15/15/15/11`, and parameter placement of about 8.98B/9.35B/9.35B/7.59B
  parameters across cuda:0/cuda:1/cuda:2/cuda:3.
...
- Review through `../research/combined_skill_groups.md` / the repo review lenses found no
  ...
  target-model parity reference, or formal G2/G3 gate closure. The validator and
  docs now encode those non-claims explicitly.
- User guide updates distinguish the fast synthetic `local-4gpu-moe-serving-smoke`
  from the real `local-real-moe-serving-smoke` command, document the exact cached
  Qwen path and validation command, and note that the real command uses cached
  tokenizer/template plus `return_audio=False` because the current environment lacks
  `qwen_omni_utils`, `torchvision`, and vLLM.
- Validation passed: `python3 -m py_compile fornax/local_real_moe_serving_smoke.py
  fornax/cli.py tests/test_fornax_planner.py`; focused real-MoE validator unit
  tests (4 tests); `python3 -m fornax program local-real-moe-serving-smoke ...`;
  `python3 -m fornax test local-real-moe-serving-smoke --fixture
  /tmp/fornax_qwen3_omni_real_moe_smoke_20260629.json`; `python3 -m unittest
  tests.test_fornax_planner` (257 tests); `make golden`; `git diff --check`; and
  final `make test` (golden suite plus 257 discovered tests) all passed.

### User guide style rewrite (2026-06-29)

- Rewrote the reader-facing guide set: `README.md`, `docs/README.md`,
  `docs/getting-started.md`, `docs/concepts.md`,
  `docs/planning-and-simulation.md`, `docs/input-formats.md`, and
  `docs/cli-reference.md`.
- Removed the LLM-styled framing the user called out: em dash/en dash punctuation,
  repeated honesty branding, stale `the-honest-constraint` anchors, ornamental
  contrast phrasing, and blog-like caveats. Replaced it with direct scope labels:
  simulator output is prediction; benchmark and serving-smoke output is measured
  data tied to the named artifact.
- Preserved the technical boundaries from the prior Qwen work: the real smoke is
  same-host Qwen3-Omni text generation evidence, while live HTTP, full multimodal
  serving, production distributed transport, target-model parity, and formal
  G2/G3 closure remain excluded from that artifact's scope.
- Validation passed: style scan over the edited guide files found no non-ASCII
  punctuation, `honest`/`honesty`, stale `the-honest-constraint` links,
  `not ... but ...` pattern, or the other targeted phrases; local Markdown
  links/anchors resolved; `git diff --check` passed; `make test` passed with the
  golden suite plus 257 discovered unit tests.

### Apple Silicon MAX/Qwen MoE addendum (2026-06-29)

- Branch: `fornax_apple_silicon`.
- Final Apple target: `Qwen/Qwen3-30B-A3B`, architecture
  `Qwen3MoeForCausalLM`, `num_experts=128`, `num_experts_per_tok=8`.
- Final MAX runtime: Pixi nightly `MAX 26.5.0.dev2026062906` with
  `Mojo 1.0.0b3.dev2026062906`, invoked as `pixi run max` from
  `/private/tmp/fornax-max-smoke.1CqC8m`.
- Final Apple hardware evidence: MacBook Pro `Mac15,8`, Apple M3 Max, 128 GB
  unified memory.
- Structured artifacts:
  `/tmp/fornax_apple_qwen3_moe_smoke.json` and
  `/tmp/fornax_apple_qwen3_moe_no_overlap_smoke.json`.
- Result: both artifacts fail validation correctly because MAX returns nonzero
  before generated text. The failure signature is `Metal Compiler failed to
  compile metallib` and `graph compiler could not elaborate the generated KGEN`.
  Retrying with `--no-enable-overlap-scheduler --force` does not bypass the
  compiler failure.
- Implementation follow-through: added the Apple Silicon MAX MoE smoke command,
  validator, Pixi `--max-cwd` support, repeatable `--max-extra-arg`, focused
  tests, README/getting-started/CLI-reference updates, and explicit non-closure
  claims. Final `make test` passes with golden/contract checks plus 261 unittest
  cases when run outside the socket-restricting sandbox.

### Apple Silicon scope clarification (2026-06-29)

- User clarified that the Apple Silicon path must remain MAX-based. Fornax is
  intended to get cross-platform compatibility through MAX, so an Apple-native
  non-MAX runtime is not an acceptable substitute for this goal.
- A temporary MLX-LM probe had been started only to test whether the M3 Max could
  execute a smaller converted Qwen MoE locally. It was interrupted before
  generation and no MLX code path was added to Fornax.
- Documentation wording was tightened so the Apple Silicon smoke is explicitly
  MAX-only and non-MAX Apple runtimes do not count as closure for this command.
- Current evidence therefore remains: MAX nightly can resolve and start a real
  Qwen MoE on this Mac, but the tested Apple GPU path fails in MAX's Metal/KGEN
  graph compilation before generated text.

### last30days setup state (2026-06-29)

- Re-read the installed `last30days` skill contract. Its first-run gate still
  reports `FIRST_RUN_DETECTED`, so the full engine cannot be run compliantly
  until the setup wizard is completed with explicit cookie/no-cookie consent.
- The skill's safe `--preflight` command produced no output after roughly one
  minute and had to be terminated by exact command match. No browser cookies were
  read and no `SETUP_COMPLETE=true` marker was written.
- MAX Apple Silicon development notes in this branch therefore remain based on
  official Modular/MAX docs, the installed MAX nightly model list, and direct
  local MAX probes rather than a completed `last30days` engine report.

### Additional MAX-only Apple MoE probes (2026-06-29)

- Improved the Apple smoke validator so it handles non-Qwen routed-expert
  metadata. Kimi/DeepSeek-style configs may use `n_routed_experts` and
  `n_shared_experts` rather than only `num_experts`, and some supported MoE
  architecture names do not contain the literal string `moe`.
- Tested `Qwen/Qwen3-30B-A3B-Instruct-2507-FP8` through MAX with
  `--quantization-encoding float8_e4m3fn`. The artifact
  `/tmp/fornax_apple_qwen3_moe_fp8_smoke.json` records
  `Qwen3MoeForCausalLM`, `num_experts=128`, `num_experts_per_tok=8`, and the
  FP8 runtime setting. It still fails before generated text with the same
  Apple `Metal Compiler failed to compile metallib` / KGEN graph compilation
  signature as the BF16 Qwen run.
- Tested `moonshotai/Kimi-VL-A3B-Instruct`, which MAX lists under
  `KimiVLForConditionalGeneration`. The artifact
  `/tmp/fornax_apple_kimi_vl_a3b_smoke.json` records `n_routed_experts=64`,
  `n_shared_experts=2`, and `num_experts_per_tok=6`, but fails before weight
  download because Hugging Face requires `trust_remote_code`.
- After explicit user authorization, retried Kimi with
  `--max-extra-arg=--trust-remote-code`. The artifact
  `/tmp/fornax_apple_kimi_vl_a3b_trust_smoke.json` gets past the trust gate but
  fails during MAX config support with `ValueError: 'yarn' is the only
  rope_scaling type currently supported`.
- Conclusion after the additional probes: current MAX nightly has multiple
  distinct Apple/MoE blockers on this M3 Max. Qwen MoE reaches graph
  compilation and fails in Apple Metal/KGEN, while Kimi-VL-A3B fails earlier on
  MAX rope-scaling support. The Fornax wrapper now captures both failure classes
  as structured evidence without substituting a non-MAX runtime.
- Verification after this pass: `git diff --check`, `python3 -m py_compile
  fornax/apple_silicon_moe_serving_smoke.py fornax/cli.py
  tests/test_fornax_planner.py`, the five focused Apple smoke validator tests,
  `python3 -m compileall -q fornax tests`, and final unsandboxed `make test`
  passed. The full suite included golden/contract checks plus 262 unittest
  cases.

### Qwen FP8 follow-up after current MAX run finished (2026-06-29)

- Waited for the current `Qwen/Qwen3-30B-A3B-Instruct-2507-FP8` MAX run to
  complete before starting any further probes. The finished artifact
  `/tmp/fornax_apple_qwen3_moe_fp8_smoke.json` records real Qwen3 MoE metadata
  (`Qwen3MoeForCausalLM`, `num_experts=128`, `num_experts_per_tok=8`) and fails
  before generated text after MAX downloads the four FP8 shards. The failure
  remains `RuntimeError: Graph compilation failed`, `Metal Compiler failed to
  compile metallib`, and `graph compiler could not elaborate the generated
  KGEN`.
- Local MAX source inspection showed the failure occurs while
  `LogProbabilitiesMixin` unconditionally constructs and loads
  `log_probabilities_ragged_graph` on the target device. This explains why the
  failure appears before request-level logprob settings can matter.
- Retried cached Qwen FP8 with `--max-extra-arg=--sample-on-host` in
  `/tmp/fornax_apple_qwen3_moe_fp8_sample_on_host_smoke.json`; it failed with
  the same Metal/KGEN graph compilation signature.
- Retried cached Qwen FP8 with `--max-extra-arg=--prefer-module-v3` in
  `/tmp/fornax_apple_qwen3_moe_fp8_prefer_module_v3_smoke.json`; it also failed
  with the same Metal/KGEN graph compilation signature, so this nightly did not
  expose a working eager/module-v3 bypass for Qwen3 MoE on this Mac.
- Checked MAX CPU fallback without leaving the MAX scope. Cached Qwen FP8 with
  `--devices cpu` failed because `float8_e4m3fn` is not compatible with CPU in
  this runtime. Cached Qwen BF16 with `--devices cpu` failed because `bfloat16`
  is not compatible with CPU. A float32 CPU retry was not run because a 30B MoE
  float32 load would likely exceed practical memory headroom on the 128 GB M3
  Max once runtime overhead is included.
- Combined-skill review note: keep the product boundary MAX-only, preserve
  hardware-specific evidence labels, avoid substituting non-MAX Apple runtimes,
  and avoid treating these artifacts as serving success because no generated
  text was produced.

### MAX serve-mode artifact for Qwen FP8 on Apple Silicon (2026-06-29)

- Added `--runtime-mode serve` to `fornax program
  apple-silicon-moe-serving-smoke`. Generate mode remains the default. Serve
  mode starts `max serve`, sends one local `/v1/chat/completions` request,
  records HTTP status/error details, and only validates if a live HTTP response
  returns generated text. The validator now distinguishes `max-generate` from
  `max-serve` and rejects serve artifacts that never produce HTTP status 200.
- Ran the cached Qwen FP8 model through the new serve mode:
  `/tmp/fornax_apple_qwen3_moe_fp8_serve_smoke.json`. Command details:
  `Qwen/Qwen3-30B-A3B-Instruct-2507-FP8`,
  `--quantization-encoding float8_e4m3fn`, `--devices gpu`, `--max-length 512`,
  `--local-files-only`, `--runtime-mode serve`, MAX
  `26.5.0.dev2026062906`, Mojo `1.0.0b3.dev2026062906`.
- Result: MAX's server supervisor starts, reports
  `architecture=Qwen3MoeForCausalLM`, `pipeline=OverlapTextGenerationPipeline`,
  `devices=gpu[0]`, and `cache_memory=24.00 GiB`, then the model worker crashes
  during pipeline initialization. The failure is again
  `RuntimeError: Graph compilation failed`, `Metal Compiler failed to compile
  metallib`, and `graph compiler could not elaborate the generated KGEN`, with
  the stack passing through `LogProbabilitiesMixin` and
  `log_probabilities_ragged_graph`.
- The serve artifact correctly fails validation with
  `live_http_endpoint=false`, `http_status=null`, `server_returncode=1`, and
  `MAX serve exited before HTTP completion`. This closes the ambiguity between
  single-shot `max generate` and actual MAX HTTP serving: both fail on the same
  Apple Metal/KGEN compiler path for this Qwen FP8 MoE on the current nightly.
- Verification for the code change used the Pixi Python interpreter because the
  host `python3` launcher hung even for `python3 --version` during this turn.
  `/private/tmp/fornax-max-smoke.1CqC8m/.pixi/envs/default/bin/python -m
  py_compile fornax/apple_silicon_moe_serving_smoke.py fornax/cli.py
  tests/test_fornax_planner.py` passed, and the seven focused Apple smoke
  validator tests passed.

### MAX Apple Silicon current-state research note (2026-06-29)

- The `last30days` skill could not be run as a full engine report because its
  first-run gate still returns `FIRST_RUN_DETECTED`; per the skill contract, a
  setup choice and cookie/no-cookie consent are required before research. I did
  not write `SETUP_COMPLETE=true`, did not read browser cookies, and did not
  fabricate a `/last30days` report.
- Official Modular docs were checked as the fallback primary source for current
  MAX platform changes. The v26.5 nightly changelog lists new MoE/model support,
  including GLM-5.2 via `GlmMoeDsaForCausalLM`, Laguna sparse-MoE support, and
  Qwen 3.5 / 3.6 tool-calling and reasoning support. It also lists Apple GPU
  kernel changes: Apple silicon MAX model support extended to M1/M2 systems
  after fixing optimized matmul correctness there, and split-K decode attention
  for Apple GPUs made the default token-generation attention path for covered
  paged-KV MHA/GQA shapes. Source:
  [Modular MAX changelog](https://docs.modular.com/max/changelog/).
- The official supported-models page continues to present supported
  architecture families as deployable through `max serve`, including
  `DeepseekV2ForCausalLM` with `deepseek-ai/DeepSeek-V2-Lite-Chat`,
  `DeepseekV3ForCausalLM`, and the Qwen/Kimi/GLM families visible in the local
  `max list --json` output. Source:
  [MAX supported models](https://docs.modular.com/max/models/).
- Interpretation: upstream Apple work is active in the MAX nightly line, but
  the official current notes do not indicate a fix for the specific
  `log_probabilities_ragged_graph` Metal/KGEN failure observed on this M3 Max.
  The local generated artifacts remain the authoritative evidence for this
  machine and model combination.
- A DeepSeek alternative was considered because local MAX lists
  `deepseek-ai/DeepSeek-V2-Lite-Chat` under both graph and module-v3
  architectures with BF16 support. It was not run in this pass because the
  machine had only about 25 GiB free on `/System/Volumes/Data`; the model was
  not cached locally, and downloading another BF16 MoE would risk filling the
  disk unless an existing cache such as the 57 GiB Qwen BF16 snapshot is removed
  with explicit approval.
- Full regression after the serve-mode change passed with
  `make test PYTHON=/private/tmp/fornax-max-smoke.1CqC8m/.pixi/envs/default/bin/python`.
  The run covered all golden/contract suites plus 264 unittest cases. Final
  `git diff --check` also passed.

### DeepSeek-V2-Lite-Chat Apple MAX run after local kernels (2026-06-30)

- Ran the source-built MAX CLI from the upstream checkout against the local
  `deepseek-ai/DeepSeek-V2-Lite-Chat` snapshot:
  `/Users/abdulmuneer/.cache/huggingface/hub/models--deepseek-ai--DeepSeek-V2-Lite-Chat/snapshots/85864749cd611b4353ce1decdb286193298f64c7`.
  The command used `--devices gpu`, BF16, `--max-length 256`,
  `--max-batch-size 1`, `--max-batch-total-tokens 256`,
  `--kv-cache-page-size 16`, `--top-k 1`, and `--max-new-tokens 1`.
- Result: not operational end-to-end yet. The CLI build was up to date, the
  DeepSeek model graph built in about 1.7 seconds, and compilation ran for about
  47.9 seconds before failing during MLA decode graph elaboration with
  `constraint failed: flareMLA_decoding currently only supports Nvidia and AMD GPUs.`
- The Apple `flare_mla_prefill` kernel is still useful progress but is not
  sufficient for generation. Full generation enters the decode path even for a
  one-token smoke, and current upstream MAX still gates `flareMLA_decoding` to
  NVIDIA/AMD.
- Added a focused Apple MoE indices fallback in the upstream MAX checkout for
  the previous prefill-only `warp.vote` blocker. The final M3-safe version is a
  one-thread serial expert-bucketing kernel: no warp operations, no shared
  memory, and no atomics. This is correctness-first and intentionally not a
  performance implementation.
- Verified the MoE fallback with:
  `./bazelw --output_user_root=/Users/abdulmuneer/workspace/fornax/external/.bazel-output test --local_resources=gpu-memory=1000 --test_output=all //max/kernels/test/gpu/nn:test_apple_moe_indices_smoke.mojo.test`.
  The test passed with `Apple MoE indices smoke passed`.
- Failed implementation attempts worth preserving: an `is_apple_gpu()` compile
  gate still instantiated the old `warp.vote` path, and earlier per-expert
  atomic fallback variants tripped Metal XPC failures on this M3 Max. The broad
  upstream MoE stress target is still too unstable for Apple; the focused smoke
  target is the reliable regression hook for now.
- Re-running DeepSeek with `--pipeline-role prefill_only` after the MoE fallback
  moved past the old `warp.vote` failure. The next prefill-only blocker is
  generic rank-2 gather lowering in the MoE permutation path, which emits
  Metal-unsupported double-precision conversion/floor instructions in kernels
  named like `gather_r2_w8_b256_gs_True_b6be364d` and
  `gather_r2_w1_b256_gs_False_c8a36a1d`.
- Current blocker split for M3:
  full generation needs an Apple `flareMLA_decoding` implementation or fallback;
  prefill-only needs an Apple-safe MoE gather/permutation path after expert
  bucketing. After those compile, the Apple MLA prefill and MoE bucketing paths
  still need numerical correctness tests and performance work before any Fornax
  serving claim is credible.

### DeepSeek-V2-Lite-Chat after Apple MLA decode fallback (2026-06-30)

- Added a first Apple MLA decode route in the upstream MAX checkout. The
  optimized split-K attempt compiled but Metal pipeline creation failed on this
  M3 Max with an XPC interruption, so the active path is a correctness-first
  fallback: call the existing Apple `mha_gpu_naive` over the full DeepSeek MLA
  cache row and copy the latent/value portion into the MLA output.
- Verified focused Apple tests after cleanup:
  `test_apple_mla_prefill_compile.mojo.test`,
  `test_apple_mla_decode_smoke.mojo.test`, and
  `test_apple_moe_indices_smoke.mojo.test` all pass. The smoke output reported
  M3 `compute_capability: 3`, finite MLA prefill/decode outputs, and
  `Apple MoE indices smoke passed`.
- Re-ran `deepseek-ai/DeepSeek-V2-Lite-Chat` with the source-built MAX
  `pipelines generate` entrypoint, BF16, GPU, `--max-new-tokens 1`, and the
  local HF snapshot. The run now gets past the earlier
  `flareMLA_decoding currently only supports Nvidia and AMD GPUs` gate.
- Current full-model blocker: MAX graph compilation still fails in generic
  rank-2 gather lowering with Metal-unsupported double-precision conversion and
  `llvm.floor.f64`, in kernels named like
  `gather_r2_w8_b256_gs_True_*` and `gather_r2_w1_b256_gs_False_*`.
- Attempted fixes that did not clear the blocker: an interpreter gather index
  decomposition tweak, a rank-parametric graph `mo.gather` Apple direct branch,
  and several stdlib integer-division experiments. The broad stdlib experiments
  were removed because they were not semantically safe and did not clear the
  verifier failure.
- Conclusion: DeepSeek-V2-Lite-Chat is still not operational on M3 GPU. The
  next useful implementation target is an Apple-safe gather/permutation path for
  the MoE graph, likely narrower than changing generic stdlib integer division.

### DeepSeek-V2-Lite-Chat M3 MAX smoke passes (2026-07-01)

- Added a direct flat GPU rank-2 axis-0 gather path in the upstream MAX checkout
  to avoid the generic gather lowering that emitted Metal-unsupported f64/floor
  instructions in the DeepSeek MoE permutation path. Verified with
  `test_apple_gather_rank2_smoke.mojo.test` for both `int32` and `uint32`
  indices.
- Added a Metal branch to `mla_decode_dispatch_scalars`: the Apple decode
  fallback is single-partition, so the helper now returns
  `(batch_size, q_max_seq_len, 1)` for `ctx.api() == "metal"` instead of
  rejecting the model at runtime.
- Re-ran the focused Apple test set:
  `test_apple_gather_rank2_smoke.mojo.test`,
  `test_apple_mla_prefill_compile.mojo.test`,
  `test_apple_mla_decode_smoke.mojo.test`, and
  `test_apple_moe_indices_smoke.mojo.test`. All four passed. The MLA smoke tests
  reported M3 `compute_capability: 3` and zero non-finite outputs.
- Re-ran the local `deepseek-ai/DeepSeek-V2-Lite-Chat` snapshot through the
  source-built MAX `pipelines generate` entrypoint on `gpu[0]` with BF16,
  `--max-length 256`, `--max-batch-size 1`, `--max-batch-total-tokens 256`,
  `--kv-cache-page-size 128`, `--top-k 1`, and prompt `Hello`.
- One-token smoke result: model graph built, model and sampler compiled and
  initialized, prompt size 8, output size 1, generated `!`, total latency about
  869 ms.
- Eight-token smoke result: prompt size 8, output size 8, generated
  `!!!!!!!!`, time to first token about 211 ms, token-generation throughput
  about 4.43 tok/s, total latency about 1.79 s.
- Important caveats: this is short-generation smoke evidence only. It does not
  prove numerical logit parity, long-context stability, batching, serving,
  memory pressure behavior, or acceptable performance. The Apple MLA decode and
  MoE bucketing paths are still correctness-first fallbacks, not optimized M3
  kernels.

## 2026-07-10 — Phase 0.5 Engine v0 execution

### Phase 0.5 implementation audit and planner defect repairs

- Audited S05-1 through S05-12 against the current Python package. The existing
  worker, transport, scheduler, stage-host, and engine modules are deterministic
  contract/fixture simulators; they do not yet implement the production Stage
  ABI codec, `StageExecutable` backend seam, persistent loopback worker channel,
  or two-process Engine v0 required by the active sprint.
- Reproduced I-7 in `fornax.planner.cost`: remote-expert selection considered
  transfer time but did not require the expert host to support the activation
  dtype or reserve memory for expert weights, concurrent dispatch/result
  buffers, routing metadata, runtime reserve, and fragmentation.
- Fixed I-7 by adding explicit remote expert-host allocation accounting, dtype
  filtering, same-node offload rejection, and whole-plan resource aggregation.
  The whole-plan check charges concurrent stage/replica assignments to the
  expert host and also accounts for any local stage allocation on that host.
- Reproduced I-8 in `fornax.planner.search`: any fleet larger than six nodes was
  collapsed to one compute-ranked node order. This discarded a slower
  high-memory node even when it was the only feasible placement.
- Fixed I-8 by using exact permutations while the bounded search space is at
  most 4096 orders, then using deterministic compute/memory/bandwidth/reliability
  orderings with forced node/position coverage. The fixed cutoff at six nodes is
  gone.
- Added regressions for insufficient expert-host memory, incompatible expert
  dtype, concurrent expert assignments exceeding shared capacity, and a
  seven-node fleet whose only feasible node is the slow high-memory node.
- Focused verification passed five tests covering both new regressions, existing
  trace-driven remote-expert behavior, and all packaged golden plans.

### Engine v0 implementation findings and closure hardening

- Implemented the Phase 0.5 production-shaped runtime seam in
  `fornax/stage_runtime.py`: canonical manifests and tensors, lifecycle/KV epoch
  enforcement, cancellation/deadline/stale-plan handling, at-most-once request
  results, a deterministic reference backend, a scenario/fault-injectable
  simulated MAX backend, and an explicitly unavailable physical MAX backend
  that never falls back silently.
- Implemented the FNX1 Stage ABI in `fornax/stage_abi.py`: the specified 40-byte
  network-order prelude, canonical duplicate-free metadata, CRC32C, payload and
  control-kind bounds, sequence/duplicate tracking, and identity validation.
  The DeepSeek-shaped golden now binds `[2,2048]`, layers 0–13 / 14–26, exact
  manifest/input/frame/final payload hashes, and 24 positive/negative
  conformance checks. `python3 -m fornax test stage-abi-v1` passes with zero
  first-16 reference error.
- Implemented two spawned worker processes, separate HTTP control and persistent
  TCP tensor planes, negotiation/heartbeat/ACK/credit/error/cancel/reconnect,
  the two-stage orchestrator, and the bounded FIFO admission scheduler in
  `fornax/engine_v0.py`. The focused Phase 0.5 suite currently passes 15 tests,
  including independent PIDs, prefill/decode, real client concurrency, RSS
  reporting, reconnect, queue bounds, and cleanup.
- Found a byte-credit accounting bug during the first closure run: the client
  replenished the configured maximum frame size after every payload, causing
  the observed balance to inflate to hundreds of GiB. Fixed replenishment to
  return only the consumed payload bytes and strengthened the evidence validator
  to reject message or byte credit above configured capacity.
- Found contract drift in the first evidence generator: it used toy hidden size
  8 and layers 0–1 / 2–3 while the mechanism target binds DeepSeek V2 Lite hidden
  size 2048 and layers 0–13 / 14–26. Corrected the manifests, replaced the stale
  golden, and made the artifact validator compare both manifests to the exact
  Phase 0.5 mechanism target so a self-consistent toy artifact cannot pass.
- Found that the first sustained artifact labeled sequential requests as
  concurrency 8 and substituted 1,800 virtual seconds for the sprint's explicit
  30-minute duration. The validator now rejects that artifact. The runner now
  uses an actual wall-clock interval, eight simultaneously admitted requests per
  wave, observed-concurrency accounting, worker peak-RSS sampling through the
  control plane, and configured credit/RSS bounds. A one-second focused proof
  passes; the authoritative 30-minute closure run remains pending at this point
  in the journal.
- Physical disposition remains explicit: the local MAX source checkout and
  source-built CLI exist, but no `StageExecutable` physical adapter and no
  qualifying two-node heterogeneous fleet are available. This is an accepted
  S05-12 `unavailable` disposition, not G2 evidence and not a reason to block
  S05-1 through S05-11.

### Phase 0.5 wall-clock closure evidence

- Completed the authoritative sustained run and wrote
  `docs/fornax/evidence/phase05-engine-v0-2026-07-10.json`, SHA-256
  `d9f57d940306568959fd87139c0e95b8dcdd770166eabc2c31e9d425d40d1e37`.
  The independent artifact validator reports all 12 Phase 0.5 exit checks
  passing while keeping `formal_g2_passed=false` and `physical_claim=false`.
- The sustained record contains 1,800.010221083 seconds of real wall-clock
  execution, 14,304 completed loopback requests in 1,788 waves, configured and
  observed concurrency 8, two independent worker PIDs, and clean worker/channel
  teardown. Prefill and decode both passed through the production ABI with
  zero reference error and identical top-1 index 7.
- Backpressure remained exactly bounded: message credit min/max/configured was
  `1/1/1`; byte credit min/max/configured was
  `268435456/268435456/268435456`. Peak observed process RSS was 124,043,264
  bytes against the explicit 1,073,741,824-byte T1 guardrail, with 62 worker RSS
  samples; neither queue nor memory bounds were exceeded.
- The same bundle records 60 named scenario rows (five SA-linked scenarios ×
  four context points × concurrency 1/4/8), seven fault outcomes, three
  scheduler sweeps, eight trace samples, channel counters, and no activation
  contents. The manifests bind hidden size 2048 and the 0–13 / 14–26 stage cut.
- Final regression verification passed `make test`: all golden/contract suites,
  the 24-check Stage ABI corpus, and 275 unit tests completed successfully.
  This closes Phase 0.5 at T0/T1 scope only; physical MAX execution, numerical
  parity across real nodes, planner calibration, and supported-platform claims
  remain G2 work.
