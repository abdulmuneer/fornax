...
- `python3 -m fornax program local-http-serving-smoke --out /tmp/fornax_local_http_serving_mtls_target_fixture_smoke_20260622.json --plan-id phase3-local-http-mtls-target-fixture-plan --plan-hash sha256:phase3-local-http-mtls-target-fixture-plan --backend-mode target-fixture --enable-mtls ...`: 12/12 checks passed with local mTLS/HTTPS `/v1/chat/completions`, verified local CA hash `sha256:e90deba81223b984e19b6920a58768cd90d79219c19b7079e0308926aae09a29`, verified local client-certificate hash `sha256:3372a09773e63bca17ea20a31fd4e078ea83313b33bc88686022cdcbcdab3d75`, client subject `fornax-local-client`, missing-client-certificate transport rejection, `mtls_verified_peer_count=8`, local `FornaxBackend` integration, local bearer-token auth rejection, deterministic 429 backpressure rejection at `max_inflight=2`, lifecycle cleanup/state ownership, local target-fixture loading, tokenizer/template hash recording, stop-sequence handling, non-stream/SSE fixture parity, plan-integrity rejection, and bad-path rejection; `target_model_parity=false`, `real_frontier_model_parity=false`, `production_tls_enabled=false`, `production_mtls_enabled=false`, `production_auth_enabled=false`, and not product/G3 gate evidence.
...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and mTLS/HTTPS/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` mTLS/HTTPS integration, localhost OpenAI-compatible HTTP/SSE behavior, local mTLS client-certificate node identity, missing-client-certificate rejection, local bearer-token auth rejection, deterministic local backpressure rejection, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, an integrated 5/5 local H100 serving-runtime bundle with split pipeline, MoE parity, and target-fixture execution parity. Real frontier target-model parity, product auth/mTLS/keying, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
- [ ] H3 `FornaxBackend` behind `Engine`; standalone OpenAI endpoint. Partial: `fornax/serving.py` simulates Ignis/OpenAI-to-engine seam, `fornax/local_http_serving_smoke.py` validates localhost OpenAI-compatible mTLS/HTTPS/SSE behavior, verified local mTLS client identity, missing-client-certificate rejection, local bearer-token auth rejection, deterministic 429 backpressure rejection, lifecycle cleanup/state ownership, plan-integrity rejection, a local `FornaxBackend` wrapper for accepted requests, and opt-in local target-fixture backend execution/parity, `fornax/target_fixture_probe.py` validates measured H100 fixture execution/parity, and `fornax/local_serving_smoke.py` validates the serving adapter plus local H100 pipeline/MoE/target-fixture runtime probes in one bundle; real target-model backend execution/parity, product auth/mTLS/keying, and production endpoint hardening are open.
...
7. Continue moving local H100 and localhost `FornaxBackend` mTLS/HTTPS/SSE validation from local target-fixture parity and integrated measured fixture execution toward real target-model loading/parity, deeper backend execution, production-grade auth/TLS/mTLS design, and distributed failure/lifecycle semantics while preserving the distinction from final T3/T4 heterogeneous lab closure.
