...
- `python3 -m fornax program local-http-serving-smoke --out /tmp/fornax_local_http_serving_transport_runtime_bundle_mtls_target_fixture_retry_header_smoke_20260623.json --plan-id phase3-local-http-transport-runtime-bundle-mtls-target-fixture-retry-header-plan --plan-hash sha256:phase3-local-http-transport-runtime-bundle-mtls-target-fixture-retry-header-plan --backend-mode target-fixture --enable-mtls --include-activation-transfer-probe --activation-transfer-backend torch --activation-transfer-torch-python /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --activation-transfer-source-device cuda:0 --activation-transfer-destination-device cuda:1 --include-runtime-probes --runtime-probe-backend torch --runtime-probe-torch-python /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --runtime-probe-source-device cuda:0 --runtime-probe-destination-device cuda:1 --include-target-fixture-execution-probe --target-fixture-execution-backend torch --target-fixture-execution-torch-python /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --target-fixture-execution-device cuda:0 ...`: 22/22 checks passed with local mTLS/HTTPS `/v1/chat/completions`, verified local client subject `fornax-local-client`, missing-client-certificate transport rejection, `mtls_verified_peer_count=13`, local `FornaxBackend` integration, backend execution count `6`, local bearer-token auth rejection, deterministic 429 backpressure rejection at `max_inflight=2` with HTTP `Retry-After: 1`, `x-fornax-retry-after-ms=25`, and body `retry_after_ms=25`, retry-after recovery with HTTP `200` after capacity clears, admitted cancellation before backend execution with HTTP `409`/`request_cancelled`, admitted timeout before backend execution with HTTP `504`/`backend_timeout`, local partition fence before backend execution with HTTP `503`/`network_partition_fenced`, local partition recovery with HTTP `200` after the fence, lifecycle accepted/cleanup count `9`, lifecycle timed-out count `1`, lifecycle partitioned count `1`, lifecycle resource allocation/release count `45`, local target-fixture loading, tokenizer/template hash recording, stop-sequence handling, non-stream/SSE fixture parity, integrated measured two-H100 activation transfer from `cuda:0` to `cuda:1` at about `199.67` GiB/s with about `0.0000783` s per transfer and max abs error `0.0`, integrated measured two-H100 split-pipeline correctness from `cuda:0` to `cuda:1` at about `3429.29` tokens/s with max abs error about `6.71e-08`, integrated measured two-H100 MoE parity from `cuda:0` to `cuda:1` at about `1452.15` tokens/s and `2904.31` expert calls/s with max logit abs error `0.0`, integrated measured H100 target-fixture execution on `cuda:0` with generated text `fixture h100`, about `5803.20` tokens/s, max abs error about `2.37e-06`, plan-integrity rejection, and bad-path rejection; `target_model_parity=false`, `real_frontier_model_parity=false`, `target_fixture_execution_real_frontier_model=false`, `production_tls_enabled=false`, `production_mtls_enabled=false`, `production_auth_enabled=false`, `distributed_cancel_evidence=false`, `production_partition_evidence=false`, and not product/G3 gate evidence.
...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and mTLS/HTTPS/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` mTLS/HTTPS integration, localhost OpenAI-compatible HTTP/SSE behavior, local mTLS client-certificate node identity, missing-client-certificate rejection, local bearer-token auth rejection, deterministic local backpressure rejection with HTTP Retry-After metadata, retry-after recovery after capacity clears, admitted cancellation cleanup before backend execution, admitted timeout cleanup before backend execution, local partition fencing before backend execution, local partition recovery after the fence, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, measured two-H100 activation transfer, split-pipeline correctness, and MoE parity inside the endpoint artifact, measured H100 target-fixture execution inside the endpoint artifact, and an integrated 5/5 local H100 serving-runtime bundle with split pipeline, MoE parity, and target-fixture execution parity. Real frontier target-model parity, product auth/mTLS/keying, distributed cancellation/partition proof, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
- [ ] M5 Heterogeneous frontier serve. Partial: planner, model support, serving adapter, trust/security, state ownership, simulation bundle, local `FornaxBackend` mTLS/HTTPS/SSE endpoint smoke with verified local client-certificate node identity, missing-client-certificate rejection, bearer-token, deterministic backpressure rejection with HTTP Retry-After metadata, retry-after recovery, admitted cancellation cleanup, admitted timeout cleanup, local partition fencing and recovery, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, integrated endpoint-level measured two-H100 activation transfer, integrated endpoint-level measured two-H100 split-pipeline correctness, integrated endpoint-level measured two-H100 MoE parity, integrated endpoint-level measured H100 target-fixture execution, standalone and integrated measured local H100 target-fixture execution parity, and local two-H100 serving/runtime smoke exist; real frontier target-model loading/parity, product auth/mTLS/keying, distributed cancellation/partition proof, and NVIDIA/AMD/Mac frontier serve for G3 are open.
...
7. Continue moving local H100 and localhost `FornaxBackend` mTLS/HTTPS/SSE validation from local target-fixture parity, admitted cancellation cleanup, admitted timeout cleanup, HTTP Retry-After metadata, retry-after recovery, local partition fencing and recovery, endpoint-level activation-transfer/split-pipeline/MoE probes, and integrated measured fixture execution toward real target-model loading/parity, deeper backend execution, production-grade auth/TLS/mTLS design, and distributed failure/lifecycle semantics while preserving the distinction from final T3/T4 heterogeneous lab closure.
