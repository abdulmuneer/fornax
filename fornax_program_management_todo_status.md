...
- `python3 -m fornax program local-http-serving-smoke --out /tmp/fornax_local_http_serving_transport_runtime_bundle_mtls_target_fixture_smoke_20260622.json --plan-id phase3-local-http-transport-runtime-bundle-mtls-target-fixture-plan --plan-hash sha256:phase3-local-http-transport-runtime-bundle-mtls-target-fixture-plan --backend-mode target-fixture --enable-mtls --include-activation-transfer-probe --activation-transfer-backend torch --activation-transfer-torch-python /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --activation-transfer-source-device cuda:0 --activation-transfer-destination-device cuda:1 --include-runtime-probes --runtime-probe-backend torch --runtime-probe-torch-python /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --runtime-probe-source-device cuda:0 --runtime-probe-destination-device cuda:1 --include-target-fixture-execution-probe --target-fixture-execution-backend torch --target-fixture-execution-torch-python /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --target-fixture-execution-device cuda:0 ...`: 21/21 checks passed with local mTLS/HTTPS `/v1/chat/completions`, verified local client subject `fornax-local-client`, missing-client-certificate transport rejection, `mtls_verified_peer_count=13`, local `FornaxBackend` integration, backend execution count `6`, local bearer-token auth rejection, deterministic 429 backpressure rejection at `max_inflight=2`, retry-after recovery with `retry_after_ms=25` and HTTP `200` after capacity clears, admitted cancellation before backend execution with HTTP `409`/`request_cancelled`, admitted timeout before backend execution with HTTP `504`/`backend_timeout`, local partition fence before backend execution with HTTP `503`/`network_partition_fenced`, local partition recovery with HTTP `200` after the fence, lifecycle accepted/cleanup count `9`, lifecycle timed-out count `1`, lifecycle partitioned count `1`, lifecycle resource allocation/release count `45`, local target-fixture loading, tokenizer/template hash recording, stop-sequence handling, non-stream/SSE fixture parity, integrated measured two-H100 activation transfer from `cuda:0` to `cuda:1` at about `231.33` GiB/s with about `0.0000675` s per transfer and max abs error `0.0`, integrated measured two-H100 split-pipeline correctness from `cuda:0` to `cuda:1` at about `2931.70` tokens/s with max abs error about `6.71e-08`, integrated measured two-H100 MoE parity from `cuda:0` to `cuda:1` at about `1871.53` tokens/s and `3743.07` expert calls/s with max logit abs error `0.0`, integrated measured H100 target-fixture execution on `cuda:0` with generated text `fixture h100`, about `5096.16` tokens/s, max abs error about `2.37e-06`, plan-integrity rejection, and bad-path rejection; `target_model_parity=false`, `real_frontier_model_parity=false`, `target_fixture_execution_real_frontier_model=false`, `production_tls_enabled=false`, `production_mtls_enabled=false`, `production_auth_enabled=false`, `distributed_cancel_evidence=false`, `production_partition_evidence=false`, and not product/G3 gate evidence.
...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and mTLS/HTTPS/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` mTLS/HTTPS integration, localhost OpenAI-compatible HTTP/SSE behavior, local mTLS client-certificate node identity, missing-client-certificate rejection, local bearer-token auth rejection, deterministic local backpressure rejection, retry-after recovery after capacity clears, admitted cancellation cleanup before backend execution, admitted timeout cleanup before backend execution, local partition fencing before backend execution, local partition recovery after the fence, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, measured two-H100 activation transfer, split-pipeline correctness, and MoE parity inside the endpoint artifact, measured H100 target-fixture execution inside the endpoint artifact, and an integrated 5/5 local H100 serving-runtime bundle with split pipeline, MoE parity, and target-fixture execution parity. Real frontier target-model parity, product auth/mTLS/keying, distributed cancellation/partition proof, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
- [ ] M5 Heterogeneous frontier serve. Partial: planner, model support, serving adapter, trust/security, state ownership, simulation bundle, local `FornaxBackend` mTLS/HTTPS/SSE endpoint smoke with verified local client-certificate node identity, missing-client-certificate rejection, bearer-token, deterministic backpressure rejection, retry-after recovery, admitted cancellation cleanup, admitted timeout cleanup, local partition fencing and recovery, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, integrated endpoint-level measured two-H100 activation transfer, integrated endpoint-level measured two-H100 split-pipeline correctness, integrated endpoint-level measured two-H100 MoE parity, integrated endpoint-level measured H100 target-fixture execution, standalone and integrated measured local H100 target-fixture execution parity, and local two-H100 serving/runtime smoke exist; real frontier target-model loading/parity, product auth/mTLS/keying, distributed cancellation/partition proof, and NVIDIA/AMD/Mac frontier serve for G3 are open.
...
- [ ] E4 Backpressure + failure semantics. Partial: transport, metrics-ledger, resilience-replay simulations cover timeout/cancel/backpressure/replay, and `fornax/local_http_serving_smoke.py` now validates deterministic localhost 429 backpressure plus retry-after recovery, admitted cancellation, admitted timeout cleanup, and local partition fencing before backend execution plus local recovery after the fence; real distributed partition/failure semantics are open.
...
- [ ] T2 Single-node accelerator. Partial: `fornax program local-accelerator-smoke`, `fornax program local-serving-smoke`, `fornax program local-http-serving-smoke`, and `fornax accelerator target-fixture-probe` exist; local H100 runs passed expert-MLP and activation-transfer smoke (3/3), extended expert/pipeline/MoE smoke (4/4), serving-adapter plus pipeline/MoE/target-fixture runtime smoke (5/5, accelerator probes 3/3), standalone measured H100 target-fixture execution parity, and localhost `FornaxBackend` mTLS/HTTPS/SSE endpoint smoke with verified local mTLS client identity, missing-client-certificate rejection, local auth, backpressure rejection, retry-after recovery, admitted cancellation cleanup, admitted timeout cleanup, local partition fencing and recovery, lifecycle cleanup/state ownership, local target-fixture backend parity, measured two-H100 activation-transfer topology probe integration, measured two-H100 split-pipeline/MoE runtime probe integration, and measured H100 target-fixture execution probe integration (21/21). Real target-model loading/parity, product auth/mTLS/keying, distributed cancellation/partition proof, and formal gate evidence remain open.
...
7. Continue moving local H100 and localhost `FornaxBackend` mTLS/HTTPS/SSE validation from local target-fixture parity, admitted cancellation cleanup, admitted timeout cleanup, retry-after recovery, local partition fencing and recovery, endpoint-level activation-transfer/split-pipeline/MoE probes, and integrated measured fixture execution toward real target-model loading/parity, deeper backend execution, production-grade auth/TLS/mTLS design, and distributed failure/lifecycle semantics while preserving the distinction from final T3/T4 heterogeneous lab closure.
