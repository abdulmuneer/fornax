...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and HTTP/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` HTTP integration, localhost OpenAI-compatible HTTP/SSE behavior, local bearer-token auth rejection, deterministic local backpressure rejection, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, split pipeline, and MoE parity. Real frontier target-model parity, product auth/TLS, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
Current next implementation lane: keep marching from local HTTP/SSE, local `FornaxBackend` integration, local auth/backpressure/lifecycle/target-fixture checks, and two-GPU logical-host serving/runtime smoke toward real target-model loading/parity and deeper backend execution evidence where this machine can produce real evidence, while clearly labeling it as local development evidence and not as final T3/T4 heterogeneous lab closure.
...
- [ ] M5 Heterogeneous frontier serve. Partial: planner, model support, serving adapter, trust/security, state ownership, simulation bundle, local `FornaxBackend` HTTP/SSE endpoint smoke with bearer-token, deterministic backpressure rejection, lifecycle cleanup/state ownership, and local target-fixture tokenizer/template/stop/parity evidence, and local two-H100 serving/runtime smoke exist; real frontier target-model loading/parity, product auth/TLS, and NVIDIA/AMD/Mac frontier serve for G3 are open.
...
- [ ] H2 Tokenizer/chat-template/stop-token seam; model support matrix. Partial: serving adapter and model-support fixtures record template/tokenizer hashes and model capability matrix, and `fornax/local_http_serving_smoke.py` target-fixture mode validates local tokenizer/template hashes, prompt tokenization, `</final>` stop handling, and non-stream/SSE parity; real target tokenizer/template support is open.
- [ ] H3 `FornaxBackend` behind `Engine`; standalone OpenAI endpoint. Partial: `fornax/serving.py` simulates Ignis/OpenAI-to-engine seam, `fornax/local_http_serving_smoke.py` validates localhost OpenAI-compatible HTTP/SSE behavior, local bearer-token auth rejection, deterministic 429 backpressure rejection, lifecycle cleanup/state ownership, plan-integrity rejection, a local `FornaxBackend` wrapper for accepted requests, and opt-in local target-fixture backend execution/parity, and `fornax/local_serving_smoke.py` validates the serving adapter plus local H100 pipeline/MoE runtime probes; real target-model backend execution/parity, product auth/TLS, and production endpoint hardening are open.
...
- [ ] T2 Single-node accelerator. Partial: `fornax program local-accelerator-smoke`, `fornax program local-serving-smoke`, and `fornax program local-http-serving-smoke` exist; local H100 runs passed expert-MLP and activation-transfer smoke (3/3), extended expert/pipeline/MoE smoke (4/4), serving-adapter plus pipeline/MoE runtime smoke (4/4), and localhost `FornaxBackend` HTTP/SSE endpoint smoke with local auth, backpressure rejection, lifecycle cleanup/state ownership, and local target-fixture backend parity (10/10). Real target-model loading/parity, product auth/TLS, and formal gate evidence remain open.
...
7. Continue moving local H100 and localhost `FornaxBackend` HTTP/SSE validation from local target-fixture parity toward real target-model loading/parity, deeper backend execution, production-grade auth/TLS design, and distributed failure/lifecycle semantics while preserving the distinction from final T3/T4 heterogeneous lab closure.
