...
- [x] T2 Single-node accelerator proxy. `fornax program local-accelerator-smoke`, `fornax program local-serving-smoke`, `fornax program local-http-serving-smoke`, and `fornax accelerator target-fixture-probe` exist; local H100 runs passed expert-MLP and activation-transfer smoke (3/3), extended expert/pipeline/MoE smoke (4/4), serving-adapter plus pipeline/MoE/target-fixture runtime smoke (5/5, accelerator probes 3/3), standalone measured H100 target-fixture execution parity, localhost `FornaxBackend` mTLS/HTTPS/SSE endpoint smoke with endpoint/runtime/topology-route integration (23/23), and the Phase 3 proxy gate packet (8/8). Real target-model loading/parity, product auth/mTLS/keying, distributed cancellation/partition proof, and formal heterogeneous lab evidence remain deferred.
