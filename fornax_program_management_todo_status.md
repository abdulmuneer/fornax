...
- `python3 -m fornax program local-http-serving-smoke --out /tmp/fornax_local_http_serving_lifecycle_smoke_20260622.json --plan-id phase3-local-http-lifecycle-plan --plan-hash sha256:phase3-local-http-lifecycle-plan ...`: 9/9 checks passed with localhost `/v1/chat/completions`, local `FornaxBackend` integration, local bearer-token auth rejection, deterministic 429 backpressure rejection at `max_inflight=2`, lifecycle cleanup/state ownership, non-stream JSON, SSE stream chunks, plan-integrity rejection, and bad-path rejection; `target_model_loaded=false`, `target_model_parity=false`, `tls_enabled=false`, `production_auth_enabled=false`, and not product/G3 gate evidence.
...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and HTTP/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` HTTP integration, localhost OpenAI-compatible HTTP/SSE behavior, local bearer-token auth rejection, deterministic local backpressure rejection, lifecycle cleanup/state ownership, split pipeline, and MoE parity. Real target-model parity, product auth/TLS, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
Current next implementation lane: keep marching from local HTTP/SSE, local `FornaxBackend` integration, local auth/backpressure/lifecycle checks, and two-GPU logical-host serving/runtime smoke toward target-model loading/parity and deeper backend execution evidence where this machine can produce real evidence, while clearly labeling it as local development evidence and not as final T3/T4 heterogeneous lab closure.
...
- [ ] H3 `FornaxBackend` behind `Engine`; standalone OpenAI endpoint. Partial: `fornax/serving.py` simulates Ignis/OpenAI-to-engine seam, `fornax/local_http_serving_smoke.py` validates localhost OpenAI-compatible HTTP/SSE behavior, local bearer-token auth rejection, deterministic 429 backpressure rejection, lifecycle cleanup/state ownership, plan-integrity rejection, and a local `FornaxBackend` wrapper for accepted requests, and `fornax/local_serving_smoke.py` validates the serving adapter plus local H100 pipeline/MoE runtime probes; real target-model backend execution/parity, product auth/TLS, and production endpoint hardening are open.
...
- [ ] T2 Single-node accelerator. Partial: `fornax program local-accelerator-smoke`, `fornax program local-serving-smoke`, and `fornax program local-http-serving-smoke` exist; local H100 runs passed expert-MLP and activation-transfer smoke (3/3), extended expert/pipeline/MoE smoke (4/4), serving-adapter plus pipeline/MoE runtime smoke (4/4), and localhost `FornaxBackend` HTTP/SSE endpoint smoke with local auth, backpressure rejection, and lifecycle cleanup/state ownership (9/9). Target-model loading/parity, product auth/TLS, and formal gate evidence remain open.
...
7. Continue moving local H100 and localhost `FornaxBackend` HTTP/SSE validation toward target-model loading/parity, deeper backend execution, production-grade auth/TLS design, and distributed failure/lifecycle semantics while preserving the distinction from final T3/T4 heterogeneous lab closure.
