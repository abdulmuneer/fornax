...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and mTLS/HTTPS/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` mTLS/HTTPS integration, localhost OpenAI-compatible HTTP/SSE behavior, local mTLS client-certificate node identity, missing-client-certificate rejection, local bearer-token auth rejection, deterministic local backpressure rejection, retry-after recovery after capacity clears, admitted cancellation cleanup before backend execution, admitted timeout cleanup before backend execution, local partition fencing before backend execution, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, measured two-H100 activation transfer, split-pipeline correctness, and MoE parity inside the endpoint artifact, measured H100 target-fixture execution inside the endpoint artifact, and an integrated 5/5 local H100 serving-runtime bundle with split pipeline, MoE parity, and target-fixture execution parity. Real frontier target-model parity, product auth/mTLS/keying, distributed cancellation/partition proof, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
- [ ] E4 Backpressure + failure semantics. Partial: transport, metrics-ledger, resilience-replay simulations cover timeout/cancel/backpressure/replay, and `fornax/local_http_serving_smoke.py` now validates deterministic localhost 429 backpressure plus retry-after recovery, admitted cancellation, admitted timeout cleanup, and local partition fencing before backend execution; real distributed partition/failure semantics are open.
...
7. Continue moving local H100 and localhost `FornaxBackend` mTLS/HTTPS/SSE validation from local target-fixture parity, admitted cancellation cleanup, admitted timeout cleanup, retry-after recovery, local partition fencing, endpoint-level activation-transfer/split-pipeline/MoE probes, and integrated measured fixture execution toward real target-model loading/parity, deeper backend execution, production-grade auth/TLS/mTLS design, and distributed failure/lifecycle semantics while preserving the distinction from final T3/T4 heterogeneous lab closure.
