...
- `python3 -m fornax program local-http-serving-smoke --out /tmp/fornax_local_http_serving_backend_smoke_20260622.json --plan-id phase3-local-http-backend-plan --plan-hash sha256:phase3-local-http-backend-plan ...`: 6/6 checks passed with localhost `/v1/chat/completions`, local `FornaxBackend` integration, non-stream JSON, SSE stream chunks, plan-integrity rejection, and bad-path rejection; `target_model_loaded=false`, `target_model_parity=false`, `tls_enabled=false`, and not product/G3 gate evidence.
...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and HTTP/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` HTTP integration, localhost OpenAI-compatible HTTP/SSE behavior, split pipeline, and MoE parity. Real target-model parity, product auth/TLS, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
Current next implementation lane: keep marching from local HTTP/SSE, local `FornaxBackend` integration, and two-GPU logical-host serving/runtime smoke toward target-model loading/parity and deeper backend execution evidence where this machine can produce real evidence, while clearly labeling it as local development evidence and not as final T3/T4 heterogeneous lab closure.
...
- [ ] M5 Heterogeneous frontier serve. Partial: planner, model support, serving adapter, trust/security, state ownership, simulation bundle, local `FornaxBackend` HTTP/SSE endpoint smoke, and local two-H100 serving/runtime smoke exist; target-model loading/parity, product auth/TLS, and NVIDIA/AMD/Mac frontier serve for G3 are open.
...
7. Continue moving local H100 and localhost `FornaxBackend` HTTP/SSE validation toward target-model loading/parity and deeper backend execution while preserving the distinction from final T3/T4 heterogeneous lab closure.
