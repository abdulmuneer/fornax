...
- `python3 -m unittest tests.test_fornax_planner`: 219 tests passed.
...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and HTTP/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` HTTP integration, localhost OpenAI-compatible HTTP/SSE behavior, local bearer-token auth rejection, deterministic local backpressure rejection, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, an integrated 5/5 local H100 serving-runtime bundle with split pipeline, MoE parity, and target-fixture execution parity. Real frontier target-model parity, product auth/TLS, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
- [ ] T2 Single-node accelerator. Partial: `fornax program local-accelerator-smoke`, `fornax program local-serving-smoke`, `fornax program local-http-serving-smoke`, and `fornax accelerator target-fixture-probe` exist; local H100 runs passed expert-MLP and activation-transfer smoke (3/3), extended expert/pipeline/MoE smoke (4/4), serving-adapter plus pipeline/MoE/target-fixture runtime smoke (5/5, accelerator probes 3/3), standalone measured H100 target-fixture execution parity, and localhost `FornaxBackend` HTTP/SSE endpoint smoke with local auth, backpressure rejection, lifecycle cleanup/state ownership, and local target-fixture backend parity (10/10). Real target-model loading/parity, product auth/TLS, and formal gate evidence remain open.
...
7. Continue moving local H100 and localhost `FornaxBackend` HTTP/SSE validation from local target-fixture parity and integrated measured fixture execution toward real target-model loading/parity, deeper backend execution, production-grade auth/TLS design, and distributed failure/lifecycle semantics while preserving the distinction from final T3/T4 heterogeneous lab closure.
