...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime, mTLS/HTTPS/SSE, and topology-route smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` mTLS/HTTPS integration, localhost OpenAI-compatible HTTP/SSE behavior, local mTLS client-certificate node identity, missing-client-certificate rejection, local bearer-token auth rejection, deterministic local backpressure rejection with HTTP Retry-After metadata, retry-after recovery after capacity clears, admitted cancellation cleanup before backend execution, admitted timeout cleanup before backend execution, local partition fencing before backend execution, local partition recovery after the fence, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, measured two-H100 activation transfer, split-pipeline correctness, MoE parity, and verified local logical-host topology route inside the endpoint artifact, measured H100 target-fixture execution inside the endpoint artifact, and an integrated 5/5 local H100 serving-runtime bundle with split pipeline, MoE parity, and target-fixture execution parity. Real frontier target-model parity, product auth/mTLS/keying, distributed cancellation/partition proof, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
Current next implementation lane: keep marching from local mTLS/HTTPS/SSE, local `FornaxBackend` integration, local mTLS/auth/backpressure with HTTP Retry-After metadata/retry-after/admitted-cancel/admitted-timeout/local-partition-fence-and-recovery/lifecycle/target-fixture checks, integrated measured local H100 activation-transfer/split-pipeline/MoE/fixture execution inside endpoint and serving-runtime bundles, local logical-host topology-route/deferred hardware explanations, and two-GPU logical-host serving/runtime smoke toward real target-model loading/parity and deeper backend execution evidence where this machine can produce real evidence, while clearly labeling it as local development evidence and not as final T3/T4 heterogeneous lab closure.
...
- [ ] M5 Heterogeneous frontier serve. Partial: planner, model support, serving adapter, trust/security, state ownership, simulation bundle, local `FornaxBackend` mTLS/HTTPS/SSE endpoint smoke with verified local client-certificate node identity, missing-client-certificate rejection, bearer-token, deterministic backpressure rejection with HTTP Retry-After metadata, retry-after recovery, admitted cancellation cleanup, admitted timeout cleanup, local partition fencing and recovery, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, integrated endpoint-level measured two-H100 activation transfer, integrated endpoint-level measured two-H100 split-pipeline correctness, integrated endpoint-level measured two-H100 MoE parity, verified endpoint-level local logical-host topology route with deferred AMD/Apple hardware explanations, integrated endpoint-level measured H100 target-fixture execution, standalone and integrated measured local H100 target-fixture execution parity, and local two-H100 serving/runtime smoke exist; real frontier target-model loading/parity, product auth/mTLS/keying, distributed cancellation/partition proof, and NVIDIA/AMD/Mac frontier serve for G3 are open.
...
- [ ] T2 Single-node accelerator. Partial: `fornax program local-accelerator-smoke`, `fornax program local-serving-smoke`, `fornax program local-http-serving-smoke`, and `fornax accelerator target-fixture-probe` exist; local H100 runs passed expert-MLP and activation-transfer smoke (3/3), extended expert/pipeline/MoE smoke (4/4), serving-adapter plus pipeline/MoE/target-fixture runtime smoke (5/5, accelerator probes 3/3), standalone measured H100 target-fixture execution parity, and localhost `FornaxBackend` mTLS/HTTPS/SSE endpoint smoke with verified local mTLS client identity, missing-client-certificate rejection, local auth, backpressure rejection with HTTP Retry-After metadata, retry-after recovery, admitted cancellation cleanup, admitted timeout cleanup, local partition fencing and recovery, lifecycle cleanup/state ownership, local target-fixture backend parity, measured two-H100 activation-transfer topology probe integration, measured two-H100 split-pipeline/MoE runtime probe integration, measured H100 target-fixture execution probe integration, and local logical-host topology-route/deferred hardware explanation integration (23/23). Real target-model loading/parity, product auth/mTLS/keying, distributed cancellation/partition proof, and formal gate evidence remain open.
