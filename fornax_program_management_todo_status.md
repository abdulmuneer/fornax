...
- `python3 -m unittest tests.test_fornax_planner`: 223 tests passed.
...
- `python3 -m fornax program local-http-serving-smoke --out /tmp/fornax_local_http_serving_runtime_bundle_mtls_target_fixture_smoke_20260622.json --plan-id phase3-local-http-runtime-bundle-mtls-target-fixture-plan --plan-hash sha256:phase3-local-http-runtime-bundle-mtls-target-fixture-plan --backend-mode target-fixture --enable-mtls --include-runtime-probes --runtime-probe-backend torch --runtime-probe-torch-python /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --runtime-probe-source-device cuda:0 --runtime-probe-destination-device cuda:1 --include-target-fixture-execution-probe --target-fixture-execution-backend torch --target-fixture-execution-torch-python /mnt/dataprocessing/venvs/aiccu_falcon_tdt/bin/python --target-fixture-execution-device cuda:0 ...`: 16/16 checks passed with local mTLS/HTTPS `/v1/chat/completions`, verified local client subject `fornax-local-client`, missing-client-certificate transport rejection, `mtls_verified_peer_count=9`, local `FornaxBackend` integration, backend execution count `4`, local bearer-token auth rejection, deterministic 429 backpressure rejection at `max_inflight=2`, admitted cancellation before backend execution with HTTP `409`/`request_cancelled`, lifecycle accepted/cleanup count `5`, lifecycle resource allocation/release count `25`, local target-fixture loading, tokenizer/template hash recording, stop-sequence handling, non-stream/SSE fixture parity, integrated measured two-H100 split-pipeline correctness from `cuda:0` to `cuda:1` at about `3802.85` tokens/s with max abs error about `6.71e-08`, integrated measured two-H100 MoE parity from `cuda:0` to `cuda:1` at about `1859.88` tokens/s and `3719.75` expert calls/s with max logit abs error `0.0`, integrated measured H100 target-fixture execution on `cuda:0` with generated text `fixture h100`, about `5250.46` tokens/s, max abs error about `2.37e-06`, plan-integrity rejection, and bad-path rejection; `target_model_parity=false`, `real_frontier_model_parity=false`, `target_fixture_execution_real_frontier_model=false`, `production_tls_enabled=false`, `production_mtls_enabled=false`, `production_auth_enabled=false`, `distributed_cancel_evidence=false`, `production_partition_evidence=false`, and not product/G3 gate evidence.
...
| Phase 3 Heterogeneous Frontier | Simulation/preparatory; local serving-runtime and mTLS/HTTPS/SSE smoke exist; true heterogeneous lab open | Planner/model-support/serving/security/state simulations exist, and local smoke now covers serving-adapter validation, local `FornaxBackend` mTLS/HTTPS integration, localhost OpenAI-compatible HTTP/SSE behavior, local mTLS client-certificate node identity, missing-client-certificate rejection, local bearer-token auth rejection, deterministic local backpressure rejection, admitted cancellation cleanup before backend execution, lifecycle cleanup/state ownership, local target-fixture tokenizer/template/stop/parity evidence, measured two-H100 split-pipeline correctness and MoE parity inside the endpoint artifact, measured H100 target-fixture execution inside the endpoint artifact, and an integrated 5/5 local H100 serving-runtime bundle with split pipeline, MoE parity, and target-fixture execution parity. Real frontier target-model parity, product auth/mTLS/keying, distributed cancellation/partition proof, NVIDIA/AMD/Mac frontier serve, and predicted-throughput proof remain open. |
...
- [ ] H3 `FornaxBackend` behind `Engine`; standalone OpenAI endpoint. Partial: `fornax/serving.py` simulates Ignis/OpenAI-to-engine seam, `fornax/local_http_serving_smoke.py` validates localhost OpenAI-compatible mTLS/HTTPS/SSE behavior, verified local mTLS client identity, missing-client-certificate rejection, local bearer-token auth rejection, deterministic 429 backpressure rejection, admitted cancellation before backend execution, lifecycle cleanup/state ownership, plan-integrity rejection, a local `FornaxBackend` wrapper for accepted requests, opt-in local target-fixture backend execution/parity, opt-in measured two-H100 split-pipeline/MoE runtime probe integration, and opt-in measured H100 target-fixture execution probe integration, `fornax/target_fixture_probe.py` validates measured H100 fixture execution/parity, and `fornax/local_serving_smoke.py` validates the serving adapter plus local H100 pipeline/MoE/target-fixture runtime probes in one bundle; real target-model backend execution/parity, product auth/mTLS/keying, distributed cancellation/partition proof, and production endpoint hardening are open.
...
7. Continue moving local H100 and localhost `FornaxBackend` mTLS/HTTPS/SSE validation from local target-fixture parity, admitted cancellation cleanup, endpoint-level split-pipeline/MoE probes, and integrated measured fixture execution toward real target-model loading/parity, deeper backend execution, production-grade auth/TLS/mTLS design, and distributed failure/lifecycle semantics while preserving the distinction from final T3/T4 heterogeneous lab closure.
